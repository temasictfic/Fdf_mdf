...
void	get_map_center(t_mlx *mlx)
{
	...
	y_max = mlx->map->lines[y_last_index]->width;
	max_line_index = max_width_index(mlx->map->lines, y_last_index);
	x_max = mlx->map->lines[max_line_index]->width;
	mlx->center.x = (mlx->map->lines[max_line_index]->points[x_max - 1]->x + mlx->map->lines[0]->points[0]->x) / 2;
	mlx->center.y = (mlx->map->lines[y_last_index]->points[y_max -1]->y + mlx->map->lines[0]->points[0]->y) / 2;
}
